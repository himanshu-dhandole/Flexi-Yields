...
const wallets = [
	...
	return (
		<header
			className={cn(
				'sticky top-0 z-50 mx-auto w-full max-w-7xl border-b border-transparent md:rounded-md md:transition-all md:ease-out',
				{
		...
		>
			...
			>
				...
				<div className="hidden items-center gap-1 md:flex">
					{links.map((link, i) => (
						<a key={i} className={cn(buttonVariants({ variant: 'ghost' }), 'px-6')} href={link.href}>
							{link.label}
						</a>
					))}
					<Button variant="outline" className="p-2.5 w-auto h-auto">
  <ConnectButton
    client={client}
    wallets={wallets}
    theme="dark"
